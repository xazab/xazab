...
"""Class for xazabd node under test"""
...
class TestNode():
    """A class for representing a xazabd node under test.
    ...
    def __del__(self):
        # Ensure that we don't leave any xazabd processes lying around after
        # the test ends
    ...
    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the xazabd process. Returns False if unable to connect."""
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. xazabd still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to xazabd")
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, partial_match=False, *args, **kwargs):
        ...
        extra_args: extra arguments to pass through to xazabd
        expected_msg: regex that stderr should match when xazabd fails

        Will throw if xazabd starts without an error.
        Will throw if an expected_msg is provided and it does not match xazabd's stdout."""
        with tempfile.SpooledTemporaryFile(max_size=2**16) as log_stderr:
            try:
                ...
                self.log.debug('xazabd failed to start: %s', e)
                self.running = False
                self.process = None
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes xazabd to shutdown, so this method takes
...
class TestNodeCLI():
    """Interface to xazab-cli for an individual node"""
    ...
    def __init__(self, binary, datadir):
        ...
        self.datadir = datadir
        self.input = None
        self.log = logging.getLogger('TestFramework.xazabcli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with xazab-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        """Run xazab-cli command. Deserializes returned string as python object."""

        pos_args = [str(arg) for arg in args]
        named_args = [str(key) + "=" + str(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same xazab-cli call"
        ...
        self.log.debug("Running xazab-cli command: %s" % command)
