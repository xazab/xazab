...
int32_t ComputeBlockVersion(const CBlockIndex* pindexPrev, const Consensus::Params& params, int algo, bool fCheckMasternodesUpgraded)
{
	...
	nVersion |= BLOCK_VERSION_SHA256D;
        ...
        break;
	case ALGO_YESPOWER:
        if (pindexPrev->nHeight <  params.v3DiffChangeHeight)
            nVersion |= BLOCK_VERSION_X11;
        else
            nVersion |= BLOCK_VERSION_YESPOWER;
        break;
        case ALGO_LYRA2:
        if (pindexPrev->nHeight < params.nWork)
            nVersion |= BLOCK_VERSION_X11;
        else
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (VersionBitsState(pindex->pprev, consensusparams, Consensus::DEPLOYMENT_V17, versionbitscache) == ThresholdState::THRESHOLD_ACTIVE) {
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    LogPrint(BCLog::BENCHMARK, "    - Xazab specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeXazabSpecific * 0.000001);
