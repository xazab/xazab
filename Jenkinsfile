...
// which are part of the Xazab repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="xazab-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /xazab-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "xazab-xazab/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from xazab-xazab/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'xazab-xazab/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from xazab-xazab/develop"
            }
          }

          if (hasCache) {
            sh "cd /xazab-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /xazab-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /xazab-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /xazab-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /xazab-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /xazab-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /xazab-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
